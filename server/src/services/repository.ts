...
export async function getDashboardData(cooperativeId = getFirstCooperativeId()) {
  ...
  const base = cooperative ?? (await prisma.cooperative.findFirst({ include: { contributions: true } }));
...
export async function listAuditLogData(cooperativeId: string) {
  ...
  return prisma.auditEvent.findMany({ where: { cooperativeId }, orderBy: { createdAt: 'desc' } });
}

export async function registerMember(input: { firstName: string; lastName: string; phoneNumber: string; bvnHash: string }) {
  if (!usingDatabase) {
    ...
    return {
      ...
      nomba: { accountCreated: true, virtualAccountCreated: verification.verified, accountRef: `va_${member.id}` },
    };
  }

  const existing = await prisma.member.findMany({ where: { bvnHash: input.bvnHash } });
  ...
  return {
    ...
    nomba: { accountCreated: true, virtualAccountCreated: existing.length === 0, accountRef: `va_${member.id}` },
  };
}
...
export async function createCooperativeData(input: { name: string; registrationNumber: string; stateName: string; cooperativeType: Cooperative['cooperativeType']; bvn?: string }) {
...
export async function createContributionData(input: { memberId: string; cooperativeId: string; amount: number; expectedAmount?: number; duplicateBvn?: boolean }) {
  ...
  broadcastFeedEvent({
    ...
    payload: { contributionId: contribution.id, cooperativeId: input.cooperativeId, riskScore: result.riskScore },
  });
...
export async function createWithdrawalRequestData(input: {
  ...
  broadcastFeedEvent({
    ...
    timestamp: new Date().toISOString(),
    payload: { withdrawalId: withdrawal.id, riskScore: result.riskScore, category: result.riskCategory },
  });
...
export async function signWithdrawalData(withdrawalId: string, signer: { memberId: string; role: 'treasurer' | 'executive1' | 'executive2' }) {
  ...
  const current = await prisma.withdrawalRequest.findUnique({ where: { id: withdrawalId } });
  ...
  broadcastFeedEvent({
    ...
    timestamp: new Date().toISOString(),
    payload: { withdrawalId, signatureCount: updated.signatureCount, role: signer.role },
  });
...
export async function reportWhistleblowerData(input: { report: string; supportingDetails?: string }) {
...
export async function createWebhookAuditData(input: { cooperativeId: string; eventType: string; description: string; metadata?: Record<string, unknown> }) {
...
export async function recalculateHealthScoreData(cooperativeId: string) {
  ...
  const score = Math.max(
    0,
    Math.round(cooperative.healthScore - alerts.reduce((total, alert) => total + Math.min(Number(alert.riskScore) * 18, 18), 0)),
  );
