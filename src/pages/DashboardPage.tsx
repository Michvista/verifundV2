...
export function DashboardPage() {
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      try {
        const parsed = JSON.parse(event.data as string) as { type: string; message: string; timestamp: string };
        setLiveFeed((current) => [
          {
  ...
  return (
    <div className="page-grid">
      <div className="main-stack">
        <section className="hero-card page-reveal">
          <div className="hero-card__copy">
            ...
            <div className="balance">{dashboard ? `₦ ${dashboard.balance.toLocaleString('en-NG', { minimumFractionDigits: 2 })}` : 'Loading balance...'}</div>
            <div className="hero-card__meta">
              ...
              <Metric label="Trust Score" value={dashboard ? String(dashboard.trustScore) : '...'} caption="Excellent" />
            </div>
          </div>
          ...
          <div className="hero-card__chart">
            ...
            <div className="hero-card__note">
              Cooperative funds are ring-fenced in a dedicated Nomba virtual account. No treasurer can touch cash directly.
            </div>
            <Sparkline values={trend} stroke="#0d7c66" fill="rgba(13,124,102,0.18)" label="Contribution trend" />
          </div>
        </section>
          ...
          <div className="table">
            ...
            {history.map((row) => (
              <div className="table__row table__row--interactive" key={row.reference}>
                ...
                <span>
                  <StatusPill tone={row.status === 'confirmed' ? 'success' : 'neutral'}>{row.status.toUpperCase()}</StatusPill>
                </span>
      ...
      <aside className="side-stack">
        ...
        <SectionCard title="Live Activity Feed" subtitle="Recent cooperative events and verification updates." className="page-reveal">
          <div className="feed">
            {feed.map((item) => (
